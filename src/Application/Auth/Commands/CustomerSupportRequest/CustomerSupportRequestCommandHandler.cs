...
public class CustomerSupportRequestCommandHandler
{
  private readonly IValidator<CustomerSupportRequestCommand> _validator;
  private readonly IEmailSender _emailSender;
  private readonly ILoginActivityLogRepository _activityLogRepository;

  public CustomerSupportRequestCommandHandler(
      IValidator<CustomerSupportRequestCommand> validator,
  ...
  public async Task Handle(CustomerSupportRequestCommand command, CancellationToken cancellationToken)
  {
    ...
    var subject = $"Customer Support - {command.Name}";
  ...
  private static string BuildEmailBody(CustomerSupportRequestCommand command)
  {
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e7e5e3;">
        <tr>
          <td style="background:#c81322;padding:22px 28px;">
            ...
            <div style="color:#ffd9dc;font-size:12px;margin-top:4px;">Customer Support</div>
          </td>
        </tr>
        <tr>
          <td style="padding:28px;">
            <p style="margin:0 0 20px;color:#1a1918;font-size:14px;line-height:1.6;">
              A user could not sign in to SkillSets and asked for help. Their details are below - reply to this email to respond to them directly.
            </p>
        ...
        <tr>
          <td style="padding:16px 28px;background:#f7f6f5;border-top:1px solid #e7e5e3;">
            <p style="margin:0;color:#6b6663;font-size:11px;line-height:1.5;">Sent automatically from SkillSets.</p>
          </td>
        </tr>
      </table>
    </div>
