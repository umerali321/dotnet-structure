...
{
    /// <inheritdoc />
    public partial class AddManagerScopeAndUsageToDashboardStats : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Two additions, both purely additive to the existing shape:
            //   1. @RestrictToManagerId - when set (a Manager viewing their own dashboard), narrows
            //      Employees/IT/NON-IT/Usage down to just that manager's own team
            //      (StudentProfiles.ManagerId), the same restrictToManagerId pattern already used by
            //      ListStudentsQueryHandler and DashboardQueryService.GetCourseLibraryUsersAsync.
            //      Manager/CompanyAdmin counts stay company-wide either way - a Manager narrowing to
            //      "how many managers report to me" isn't a meaningful question here.
            //   2. TotalCourseUsage - total CourseTakens rows (every course any of the scoped
            //      employees has ever started, active or completed) across the scoped employee set.
...
BEGIN
    ...
    UsageAgg AS (
        -- Every course any scoped employee has ever taken, active or completed - matches
        -- employee A took 3, B took 4, C took 1, total usage 9 exactly (a plain count of
        -- CourseTakens rows), not a distinct-course count.
