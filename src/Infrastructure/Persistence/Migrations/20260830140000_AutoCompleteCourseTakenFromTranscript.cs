...
{
    /// <inheritdoc />
    public partial class AutoCompleteCourseTakenFromTranscript : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Completion for a Course Library launch now comes from the imported Skillport
            // transcript instead of the employee pressing "Mark Complete" by hand. Adds one step to
            // the import: any active CourseTaken row whose course Skillport reports as Completed is
            // closed out.
            //
            // This matters beyond tidiness - TakeCourse permits only one active course per person,
            // and with the manual button gone nothing else clears that flag, so an employee whose
            // launch is never closed cannot start another course at all.
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Close out any Course Library launch that Skillport now reports as finished. This is the
        -- ONLY thing that completes a CourseTaken row since the manual ""Mark Complete"" button was
        -- removed, and TakeCourse allows a person just one active course at a time - so without
        -- this an employee stays locked out of starting anything new.
        UPDATE ct
        SET ct.IsActive = 0,
            ct.CompletedAt = @Now
        ...
        WHERE ct.IsActive = 1
          AND EXISTS (
              ...
              WHERE a.IsLatest = 1
                ...
                AND i.UserId = ct.UserId
                AND s.InternalCourseId = ct.CourseId
          );
