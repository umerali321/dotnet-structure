...
public class StudentRepository : IStudentRepository
{
    ...
    public async Task RemoveEmployeeRoleAsync(int userId, int companyId, CancellationToken cancellationToken = default)
    {
        var today = DateOnly.FromDateTime(DateTime.UtcNow);
        var activeRoles = await _dbContext.UserCompanyRoles
            .Where(ucr => ucr.UserId == userId && ucr.CompanyId == companyId && ucr.IsActive && ucr.Role.RoleName == "Student")
